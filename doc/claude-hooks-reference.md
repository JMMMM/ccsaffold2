...
使用 PreToolUse 决策控制 来允许、拒绝或请求使用工具的权限。

### PermissionRequest

在向用户显示权限对话框时运行。

使用 PermissionRequest 决策控制 代表用户允许或拒绝。
...
- **退出代码 0**：成功。`stdout` 在详细模式（ctrl+o）中显示给用户，除了 `UserPromptSubmit` 和 `SessionStart`，其中 stdout 被添加到上下文中。`stdout` 中的 JSON 输出被解析为结构化控制（请参阅 高级：JSON 输出）。
- **退出代码 2**：阻止错误。仅使用 `stderr` 作为错误消息并反馈给 Claude。格式为 `[command]: {stderr}`。`stdout` 中的 JSON **不**被处理为退出代码 2。
...
Claude Code hooks 与 Model Context Protocol (MCP) 工具无缝协作。当 MCP 服务器提供工具时，它们以特殊命名模式出现，您可以在 hooks 中匹配。
