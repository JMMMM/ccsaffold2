...
const DEFAULT_CONFIG = {
  ...
  thinking: { type: 'enabled' }  // 启用深度思考，提高分析质量
};
 ...
 * @returns {Promise<{reasoning: string, skills: Object[]|null}|null>} Promise resolving to {reasoning, skills} or null on error
 */
async function analyze(transcriptContent, apiKey) {
  ...
  const requestBody = JSON.stringify({
    ...
    thinking: DEFAULT_CONFIG.thinking,  // 启用深度思考
    messages: [
      {
        role: 'user',
        content: prompt
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        try {
          ...
          // 智谱 AI API 响应格式: response.choices[0].message
          // 深度思考模式下包含 reasoning_content（思考过程）和 content（最终答案）
          if (response.choices && response.choices[0] && response.choices[0].message) {
            ...
            const parsedResults = parseResponse(content);
            const validSkills = filterValidResults(parsedResults);

            // 始终返回思考过程，skills 可能为 null
            resolve({
              reasoning: reasoning,
              skills: validSkills && validSkills.length > 0 ? validSkills : null
            });
